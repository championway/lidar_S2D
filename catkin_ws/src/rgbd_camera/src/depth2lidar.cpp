...
DEPTH2LIDAR::DEPTH2LIDAR(){
  ...
  sub_data = nh.subscribe<s2d_msgs::S2D_ImageList>("/gan_data", 1, &DEPTH2LIDAR::callback, this);
}
...
void DEPTH2LIDAR::callback(const s2d_msgs::S2D_ImageListConstPtr& msg){
  ...
  for (int i = 0; i < msg->size; i++){
    PointCloudXYZRGB::Ptr cloud_patch(new PointCloudXYZRGB);
    fx = msg->list[i].fx;
    fy = msg->list[i].fy;
    cx = msg->list[i].cx;
    cy = msg->list[i].cy;
    ...
    for (int n = 0; n < 4; n++){
      for (int m = 0; m < 4; m++){
        transform_matrix(n, m) = msg->list[i].transform_matrix[n*4 + m];
      }
    }
        ...
        if (img_ptr_depth->image.at<unsigned short int>(nrow,ncol) > 1){
          ...
          if(is_gazebo){
            ...
            z = float(img_ptr_depth->image.at<unsigned short int>(nrow,ncol))/655.;
         	...
         	point.g = int(0);
         	point.b = int(255);
         	cloud_patch->points.push_back(point);
         	free(x);
         	free(y);
        }
      }
    }
    pcl::transformPointCloud(*cloud_patch, *cloud_patch, transform_matrix.inverse());
    *pc = *pc + *cloud_patch; 
  }
  sensor_msgs::PointCloud2 object_cloud_msg;
  toROSMsg(*pc, object_cloud_msg);
  if(is_gazebo){
    object_cloud_msg.header.frame_id = "/velodyne_16";
  }
  else{
